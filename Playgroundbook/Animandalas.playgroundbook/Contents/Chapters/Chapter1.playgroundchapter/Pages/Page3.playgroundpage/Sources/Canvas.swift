...
public class Canvas: UIViewController, DrawingDelegate {
    ...
    private func setupSelectedButton(_ button: UIButton){
        for view in self.view.subviews {
            if let colorButton = view as? ColorButton {
        ...
        if let selectedButton = button as? ColorButton {
            selectedButton.isSelected = true
            selectedButton.layer.borderWidth = 5
            if selectedButton.color == #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1) {
    ...
    private func presentRedrawController(){
        ...
        self.redrawController.backgroundColor = drawingView.backgroundColor!
